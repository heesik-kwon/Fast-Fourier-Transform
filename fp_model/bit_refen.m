...
function [fft_out, module2_out] = fft_fixed_stu(fft_mode, fft_in)
 ...
 bfly00_out0 = din(1:256) + din(257:512);    %%<10> = %<9> +%<9>
 bfly00_out1 = din(1:256) - din(257:512);    %%<10>= %<9> + %<9>

 bfly00_tmp = [bfly00_out0, bfly00_out1];

 for nn=1:512
   bfly00(nn) = bfly00_tmp(nn)*fac8_0(ceil(nn/128)); %<10>
 end
 ...
 for kk=1:2
  for nn=1:128
   bfly01_tmp((kk-1)*256+nn) = bfly00((kk-1)*256+nn) + bfly00((kk-1)*256+128+nn);  %<11> = %<10> + %<10>
   bfly01_tmp((kk-1)*256+128+nn) = bfly00((kk-1)*256+nn) - bfly00((kk-1)*256+128+nn);  %<11> = %<10> + %<10>
  end
 end
 ...
 for nn=1:512
   temp_bfly01(nn) = bfly01_tmp(nn)*fac8_1(ceil(nn/64));   %21 = %<11> * %<10>
   bfly01(nn) = round(temp_bfly01(nn)/256);    %12 = %<21>
 ...
 for kk=1:4
  for nn=1:64
   bfly02_tmp((kk-1)*128+nn) = bfly01((kk-1)*128+nn) + bfly01((kk-1)*128+64+nn);      %<13> = <12> + <12>
   bfly02_tmp((kk-1)*128+64+nn) = bfly01((kk-1)*128+nn) - bfly01((kk-1)*128+64+nn);  %<13> = <12> + <12>
  end
 end
 ...
 for kk=1:8
  for nn=1:64
   ...
   twf_m0((kk-1)*64+nn) = round(flo_twf_m0((kk-1)*64+nn)*128); % twf_m0 : <10>
  end
 end

 for nn=1:512
   %bfly02(nn) = bfly02_tmp(nn)*twf_m0(nn); % Org 
   pre_bfly02(nn) = bfly02_tmp(nn)*twf_m0(nn); % (14bit(13+1) * 10bit = 23 bit) 24 <= <13> * <10>
 end
 ...
 for kk=1:8
  for nn=1:32
   bfly10_tmp((kk-1)*64+nn) = bfly02((kk-1)*64+nn) + bfly02((kk-1)*64+32+nn);      % <12>  <=  <11> + <11>
   bfly10_tmp((kk-1)*64+32+nn) = bfly02((kk-1)*64+nn) - bfly02((kk-1)*64+32+nn);  % <12>  <=  <11> + <11>
  end
 end
 ...
 for kk=1:8
  for nn=1:64
   bfly10((kk-1)*64+nn) = bfly10_tmp((kk-1)*64+nn)*fac8_0(ceil(nn/16));  %  (12 bit)
  end
 end
 ...
 for kk=1:16
  for nn=1:16
   bfly11_tmp((kk-1)*32+nn) = bfly10((kk-1)*32+nn) + bfly10((kk-1)*32+16+nn); % <13>  <=  <12> + <12>
   bfly11_tmp((kk-1)*32+16+nn) = bfly10((kk-1)*32+nn) - bfly10((kk-1)*32+16+nn);% <13>  <=  <12> + <12>
  end
 end

 for kk=1:8
  for nn=1:64
   temp_bfly11((kk-1)*64+nn) = bfly11_tmp((kk-1)*64+nn)*fac8_1(ceil(nn/8)); % <23>  <=  <13> * <10>
 ...
 for kk=1:32
  for nn=1:8
   bfly12_tmp((kk-1)*16+nn) = bfly11((kk-1)*16+nn) + bfly11((kk-1)*16+8+nn); % <15>  <=  <14> + <14>
   bfly12_tmp((kk-1)*16+8+nn) = bfly11((kk-1)*16+nn) - bfly11((kk-1)*16+8+nn);% <15>  <=  <14> + <14>
  end
 end
 ...
 for kk=1:8
  for nn=1:8
   flo_twf_m1((kk-1)*8+nn) = exp(-j*2*pi*(nn-1)*(K2(kk))/64);  %<10>
   twf_m1((kk-1)*8+nn) = round(flo_twf_m1((kk-1)*8+nn)*128); % twf_m0 :%<10>
  end
 end

 for kk=1:8
  for nn=1:64
   pre_bfly12((kk-1)*64+nn) = bfly12_tmp((kk-1)*64+nn)*twf_m1(nn); % (16bit(15+1) * 10bit = 26 bit) 
  end
 end
 ...
 for kk=1:64
  for nn=1:4
   bfly20_tmp((kk-1)*8+nn) = bfly12((kk-1)*8+nn) + bfly12((kk-1)*8+4+nn);   % <13>  <=  <12> + <12>
   bfly20_tmp((kk-1)*8+4+nn) = bfly12((kk-1)*8+nn) - bfly12((kk-1)*8+4+nn); % <13>  <=  <12> + <12>
  end
 end

 for kk=1:64
  for nn=1:8
   bfly20((kk-1)*8+nn) = bfly20_tmp((kk-1)*8+nn)*fac8_0(ceil(nn/2)); % <13>
  end
 end
 ...
 for kk=1:128
  for nn=1:2
   bfly21_tmp((kk-1)*4+nn) = bfly20((kk-1)*4+nn) + bfly20((kk-1)*4+2+nn); %   % <14>  <=  <13> + <13>
   bfly21_tmp((kk-1)*4+2+nn) = bfly20((kk-1)*4+nn) - bfly20((kk-1)*4+2+nn);%   % <14>  <=  <13> + <13>
  end 
 end
 ...
 for kk=1:64
  for nn=1:8
   temp_bfly21((kk-1)*8+nn) = bfly21_tmp((kk-1)*8+nn)*fac8_1(nn);   % <24>  <=  <14> * <10>
 ...
 for kk=1:256
   bfly22_tmp((kk-1)*2+1) = bfly21((kk-1)*2+1) + bfly21((kk-1)*2+2);  % <16>  <=  <15> + <15>
   bfly22_tmp((kk-1)*2+2) = bfly21((kk-1)*2+1) - bfly21((kk-1)*2+2); % <16>  <=  <15> + <15>
 end
